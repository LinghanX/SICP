...
;;f, that itself takes one input. The result returned by make-monitored is a
;;third procedure, say mf, that keeps track of the number of times it has been
;;called by maintaining an internal counter. If the input to mf is the special
;;symbol how-many-calls?, then mf returns the value of the counter. If the input
;;is the special symbol reset-count, then mf resets the counter to zero. For any
...
;; make-monitored: function -> function
